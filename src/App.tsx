...
const App = () => {
  ...
  const onNewImageHandlerRef = useRef<((details: { path: string; previewUrl: string; source: 'file' | 'drop' | 'screenshot'; }) => void) | null>(null);
  ...
  const {
    ...
    onNewImage: (details) => {
      onNewImageHandlerRef.current?.(details);
    }
  });
  ...
  }, [activeSessionId, imagePath, imagePreviewUrl, processedImageUrl, ocrText, optimizedText, textDisplayMode, isRestoringSessionRef]);
  ...
  }, [automationSettings.autoGenerateImage, handleGenerateImage, isAutomationLoading, isGenerating, optimizedPrompt]);
  ...
  const handleSelectSession = useCallback((sessionId: string) => {
    ...
    const restoreSession = async () => {
      ...
      setActiveSessionId(sessionId);
      setHasPerformedOcr(Boolean(session.ocr.ocrText));
